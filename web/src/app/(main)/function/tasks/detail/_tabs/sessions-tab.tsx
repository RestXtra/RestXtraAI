...
import type { Activity, InterceptApprovalRow, Session, SessionStatus, TaskNode, TokenTotal } from "@/lib/types";
...
export function SessionsTab({ taskId }: { taskId: string }) {
  ...
  // Load history over REST first so the transcript is not blocked by proxies that
  // delay EventSource.onopen. SSE then tails from the returned cursor.
  React.useEffect(() => {
    let alive = true;
    ...
    const connect = (since: number) => {
      if (!alive) return;
      es = new EventSource(
        sseUrl(`/api/exploration/activity/stream?task=${encodeURIComponent(taskId)}&since=${since}`),
      ...
      es.onmessage = (e) => {
        try {
          ...
          setAllActivity((prev) =>
            prev.some((x) => x.seq === a.seq) ? prev : [...prev, a].sort((p, q) => p.seq - q.seq),
          );
        } catch {
    ...
    api
      .activity(taskId, { limit: 2000 })
      .then((r) => {
        if (!alive) return;
        setAllActivity(r.items);
        setLoaded(true);
        connect(r.cursor);
      })
      .catch(() => {
  ...
  const workerSessions = React.useMemo(() => intents.map(intentToSession), [intents]);
  ...
  const activity = React.useMemo(() => {
    ...
    const steps = allActivity.filter((a) => a.intent_id === active.intent_id || a.worker === active.id);
      ...
      <div className="grid h-[calc(100vh-13rem)] grid-cols-1 gap-4 lg:grid-cols-[18rem_1fr]">
        ...
        <div className="flex flex-col overflow-hidden rounded-lg border bg-card">
          <div className="border-b px-3 py-2">
            <div className="font-medium text-muted-foreground text-xs">会话列表</div>
            {taskTokens && (
              <div
          ...
          <ScrollArea type="auto" className="[&_[data-slot=scroll-area-viewport]>div]:block! min-h-0 flex-1">
            <div className="flex w-full flex-col gap-3 p-2">
              ...
              {loaded && !workerSessions.length && (
                <div className="px-2 py-1 text-muted-foreground text-xs">暂无运行中的 Worker 会话。</div>
              )}
            </div>
          </ScrollArea>
        </div>
        ...
        <div className="flex min-w-0 flex-col overflow-hidden rounded-lg border bg-card">
          <div className="flex items-center gap-2 border-b px-4 py-2.5">
            {(() => {
              ...
              const titleEl = <span className="font-medium text-sm">{title}</span>;
              return meta?.json ? (
                <Tooltip>
