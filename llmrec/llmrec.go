...
// persists them to PostgreSQL for later inspection (LLM 录制).
package llmrec
...
// mergeUsage merges non-zero token fields so a later event (message_delta) that
// only carries output tokens doesn't wipe the input tokens from message_start.
