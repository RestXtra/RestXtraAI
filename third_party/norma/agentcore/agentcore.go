...
type Options struct {
	...
	// Summarizer, when non-nil, overrides the default (provider-based) summarizer
	// used by compaction — e.g. a deterministic digest that avoids an extra LLM call
	// (RestXtraAI P7.3). Nil = default provider summarizer.
...
func NewSession(opts Options) *Session {
	...
	if opts.Compaction != nil {
		if opts.Summarizer != nil {
			s.compactor = compaction.New(*opts.Compaction, opts.Summarizer)
		} else {
