# RestXtraAI 项目优化问题与解决方案复盘

> 文档范围：从项目基础运行时、安全边界、Agent Harness、上下文压缩、多 Agent 协作、工具调用、前端体验到可重复性能基线的完整优化过程。
> 当前基准提交：`a7210b5 fix: isolate sandbox lifecycle from postgres`
> 说明：本文只把有代码、测试或构建记录支撑的内容写成“已完成”。没有真实 TSecBenchmark 结果数据的部分，不宣称已经获得具体百分比性能提升。
...
| 前端构建 | Next.js 静态导出 | `NEXT_EXPORT=1 next build` |
