...
type intentItem struct {
	...
	AccountScopes    []string          `json:"account_scopes"`
	RateLimitDomains []string          `json:"rate_limit_domains"`
}
...
func (t *ToolSet) addOneIntent(it intentItem) (int64, error) {
	...
	payload["rate_limit_domains"] = it.RateLimitDomains
	if len(anchors) > 0 {
...
func (t *ToolSet) addIntent() actool.CoreTool {
	return writeTool("add_intent", "生成【探索方向】写入 frontier，并连入探索链路。意图是开放的探索方向，不是固定类型——用 summary 一句话自由描述要探索/验证/利用什么。\n"+
		...
		obj(map[string]any{
			...
			"concurrency_class":  str("资源并发模式：shared 仅用于只读/低风险枚举；登录态、写操作、漏洞验证或限流敏感操作必须用 exclusive。留空时系统按 intent_class 保守推断。"),
			"account_scopes":     map[string]any{"type": "array", "items": map[string]any{"type": "string"}, "description": "会被本意图使用的账号或登录态标识；相同标识的 exclusive 意图串行。"},
			"rate_limit_domains": map[string]any{"type": "array", "items": map[string]any{"type": "string"}, "description": "共享限流域（通常为主机名/API 域）；exclusive 意图在同一限流域串行。"},
		}),
		func(_ context.Context, in json.RawMessage) (actool.Result, error) {
			var a struct {
