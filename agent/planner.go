...
// prompt every round; full text is one get_worker_output call away.
...
4. 用【一次】 add_intent 批量提交第 3 步筛出的新方向（intents 数组，最多 4 个最高价值的；不要逐条多次调）：
   - summary：一句话自由描述该方向（做什么+为什么），用自然语言。方向要在 summary 里写清楚。
   - intent_class：填写稳定、简短的测试类别（如 recon、endpoint-enum、auth、access-control、injection、ssrf、file-read、privilege-escalation）。同类方向必须复用同一值；系统会按 intent_class + asset_ids 熔断连续零产出方向。
...
func (p *Planner) Plan(ctx context.Context, taskID int64, as *db.AssetStore, ts *db.ExplorationStore, goal string, triggers []TriggerEvent, emit func(db.Activity)) (met bool, reason string, err error) {
	...
	// 态势（实际变动 + 完整图）现在拼进本轮 user 输入（见下方 input）。user 里还有
