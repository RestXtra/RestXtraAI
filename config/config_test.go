...
func TestPostgresDSNPrecedence(t *testing.T) {
	...
	t.Setenv("RESTXTRA_CONFIG", cfgPath)

	// no env DSN 鈫?built from config file fields
	t.Setenv("RESTXTRA_PG_DSN", "")
	...
	t.Setenv("RESTXTRA_PG_DSN", "postgres://envwins/x")
	if got, _, err := PostgresDSN(); err != nil || got != "postgres://envwins/x" {
		t.Fatalf("env should win, got %q err %v", got, err)
	}

	// no env, no file 鈫?error (no built-in default)
	t.Setenv("RESTXTRA_PG_DSN", "")
	t.Setenv("RESTXTRA_CONFIG", filepath.Join(dir, "nope.json"))
